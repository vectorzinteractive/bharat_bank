    ...
    <table class="mtable table table-hover data-custom-table downloads-table" id="data-table">
        <thead>
            <tr class="tablehead">
                ...
                <th class="sr-no">#</th>
                <th class="des-head sortable sort-column" data-column="town_id" data-order="{{ $currentSort === 'town_id' && $currentOrder === 'asc' ? 'desc' : 'asc' }}"> Town @if($currentSort === 'town_id') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
                <th class="des-head sortable sort-column" data-column="pincode" data-order="{{ $currentSort === 'pincode' && $currentOrder === 'asc' ? 'desc' : 'asc' }}">  Pincode @if($currentSort === 'pincode') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
        ...
        <tbody>
            ...
            <tr data-id="{{ $dataValue->id }}">
                ...
                <td class="title-and-cat-wrap col-des">{{ $dataValue->town->name ?? '' }}</td>
                <td class="title-and-cat-wrap col-des">{{ $dataValue->pincode ?? '' }}</td>
                <td class="title-and-cat-wrap col-date">
                    {{ $dataValue->created_at->format('d F Y') }}
                </td>
                <td class="col-actions">

                    <a href="{{ url('cms-admin/pincodes/' . $dataValue->id . '/edit') }}" class="edit action-edit" title="Edit" data-bs-toggle="tooltip" data-bs-placement="top">
