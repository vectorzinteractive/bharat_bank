    ...
    <table class="mtable table table-hover data-custom-table downloads-table" id="data-table">
        <thead>
            <tr class="tablehead">
                ...
                <th class="sr-no">#</th>
                <th class="des-head sortable sort-column" data-column="name" data-order="{{ $currentSort === 'name' && $currentOrder === 'asc' ? 'desc' : 'asc' }}"> Town @if($currentSort === 'name') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
                <th class="des-head sortable sort-column" data-column="city_id" data-order="{{ $currentSort === 'city_id' && $currentOrder === 'asc' ? 'desc' : 'asc' }}"> City @if($currentSort === 'city_id') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
        ...
        <tbody>
            ...
            <tr data-id="{{ $dataValue->id }}">
                ...
                <td class="title-and-cat-wrap col-des">{{ $dataValue->city->name ?? 'Uncategorized' }}</td>
                <td class="title-and-cat-wrap col-date">
                    {{ $dataValue->created_at->format('d F Y') }}
                </td>
                <td class="col-actions">

                    <a href="{{ url('cms-admin/towns/' . $dataValue->id . '/edit') }}" class="edit action-edit" title="Edit" data-bs-toggle="tooltip" data-bs-placement="top">
