    ...
    <table class="mtable table table-hover data-custom-table downloads-table" id="data-table">
        <thead>
            <tr class="tablehead">
                ...
                <th class="sr-no">#</th>
                <th class="des-head sortable sort-column" data-column="name" data-order="{{ $currentSort === 'name' && $currentOrder === 'asc' ? 'desc' : 'asc' }}"> City @if($currentSort === 'name') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
                <th class="des-head sortable sort-column" data-column="state_id" data-order="{{ $currentSort === 'state_id' && $currentOrder === 'asc' ? 'desc' : 'asc' }}"> State @if($currentSort === 'state_id') <i class="las la-arrow-{{ $currentOrder === 'asc' ? 'up' : 'down' }}"></i> @endif </th>
        ...
        <tbody>
            ...
            <tr data-id="{{ $dataValue->id }}">
                ...
                <td class="title-and-cat-wrap col-des">{{ $dataValue->state->name ?? 'Uncategorized' }}</td>
                <td class="title-and-cat-wrap col-date">
                    {{ $dataValue->created_at->format('d F Y') }}
                </td>
                <td class="col-actions">

                    <a href="{{ url('cms-admin/auction-cities/' . $dataValue->id . '/edit') }}" class="edit action-edit" title="Edit" data-bs-toggle="tooltip" data-bs-placement="top">
